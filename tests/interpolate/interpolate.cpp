...
int main(int argc, char **argv) {
  ...
  normalize(x, y, c) = interpolated[0](x, y, c) / interpolated[0](x, y, 3);
    ...
    for(int l = 0; l < levels; ++l) {
      downsampled[l].profile(PROFILE_PRODUCTION, true, true);
      interpolated[l].profile(PROFILE_PRODUCTION, true, true);
      normalize.profile(PROFILE_PRODUCTION, true, true);
    }
...
#ifdef PROFILE
    normalize.compile_to_lowered_stmt("html/interpolate_" + schedule +  ".html", {input}, HTML);
#endif

    normalize.compile_to_static_library("interpolate_aot", {input}, "normalize");

#else

    normalize.realize(out);

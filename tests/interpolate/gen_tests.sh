...
for sched in ${SCHEDULES}; do
  if [ "$sched_id" -ne "3" ]; then
    ...
    for i in $(seq 1 30); do ./interpolate_aot | tee -a csv/interpolate_${sched}_profile_$(hostname)_${IMG_SIZE}.csv ; done

    # Time tests
    echo "Running time tests for serial ${sched} schedule..."
    make clean && make SCHEDULE=${sched_id} ${EXTRA_FLAGS}
    for i in $(seq 1 30); do ./interpolate_aot | tee -a csv/interpolate_${sched}_time_$(hostname)_${IMG_SIZE}.csv ; done
  else
    # Profiler tests
    ...
    for i in $(seq 1 30); do /home/soft/likwid/bin/likwid-pin -c 3 ./interpolate_aot | grep -v likwid-pin | tee -a csv/interpolate_${sched}_profile_$(hostname)_${IMG_SIZE}.csv ; done

    # Time tests
    echo "Running time tests for parallel ${sched} schedule..."
    make clean && make SCHEDULE=${sched_id} PARALLEL=y ${EXTRA_FLAGS}
    for i in $(seq 1 30); do /home/soft/likwid/bin/likwid-pin -c 3 ./interpolate_aot | grep -v likwid-pin | tee -a csv/interpolate_${sched}_time_$(hostname)_${IMG_SIZE}.csv ; done
  fi
